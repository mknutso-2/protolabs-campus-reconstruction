...
The [portable final-viewer receipt](../research/v07c-final-viewer-review.json) binds the 12 payload assets, completed build and actual movie-panel check. Workspace evidence is `work/site-final-stage.json` and `work/site-final-build.log`. The standalone local Site checkout is `4514c7ba462d9a2ed2ec98725c3f58accb4a52c0`. Its prepared Site archive was checked against all 32 build payload files plus normalized hosting metadata: 22,469,782 bytes, SHA-256 `904a6910f77c3e9529ac364f3916e80b135b55e5aaed3af68df4bc81175fb0c3`, recorded in `work/site-final-v1-archive-validation.json`. This is the prepared hosting archive, not the versioned project-delivery ZIP. The versioned project ZIP also passed [extraction and a fresh viewer build](delivery-verification.md), with all 202 original files and all 12 runtime assets unchanged. At this local packaging checkpoint, private-site publication still required separate approval and no push, upload or deployment had occurred. That historical status is superseded by the later user-authorized public deployment recorded below.

## Public deployment — September 8, 2026 UTC

The user subsequently authorized public Sites access. [Version 1](https://protolabs-campus-mknutso.mknutso2.chatgpt.site) deployed successfully at 03:20:18 UTC, with all 12 inspected production assets unchanged. Two links now open `/accuracy.html`, which presents the existing accuracy table and public source links without requiring access to the private GitHub repository. The public build passed. Subsequent unauthenticated GET checks passed for the root page, accuracy route and all 12 runtime assets, with every asset hash/size unchanged. The deployed page opened in the browser; a fresh complete live-site interaction pass is not claimed at this checkpoint. [The publication record](site-publication.md) distinguishes the deployed bytes, anonymous HTTP verification and actual browser checks from the preserved local observations above. The 0.1.0 archive and its receipts were not regenerated.
