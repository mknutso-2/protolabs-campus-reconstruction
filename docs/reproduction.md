...
Run the commands below from the repository root unless a block explicitly changes directory. They describe the current workflow, including the clean `e29ad4b9` static source and later gentler camera path. **The [v07c static build and four-view inspection](fresh-checkout-v07c.md) are complete, and the [motion sample and endpoints](../research/v07c-motion-sample-review.json) are accepted for full-film rendering. The [complete film](../research/v07c-cinematic-review.json) is accepted with documented limits, and final local movie integration passed. An actual versioned archive passed extraction and a fresh viewer installation/build; see [delivery verification](delivery-verification.md).** A command or output file does not establish that its result has passed inspection.

The v06 master, both models, four stills, camera/material metadata and snapshot hashes are preserved in `deliverables/iterations/v06/`. Motion was deliberately not rendered for v06 because the reviewed stills exposed synthetic materials and lighting. The earlier [v07 frontage checkpoint](fresh-checkout-v07.md) and isolated regional-ground, water and parking corrections have their own evidence records. The later v07c static inspection and sample acceptance supersede their pending statuses for those specific checks. The complete film passed encoding, full decoding, actual playback and independent image review and was accepted with documented limits at 01:55:53 UTC on September 8, 2026. The [public walkthrough](https://protolabs-campus-mknutso.mknutso2.chatgpt.site) was deployed on September 8, 2026 with the user's explicit authorization; [the publication record](site-publication.md) identifies the saved version and verification scope. GitHub remains private. The versioned archive workflow passed the [documented extraction/build check](delivery-verification.md).
...
To inspect the delivered hosted copy, open the [public walkthrough](https://protolabs-campus-mknutso.mknutso2.chatgpt.site); its [publication record](site-publication.md) is separate from the immutable local 0.1.0 archive. To inspect a local build, open the local URL printed by the development server; do not open `page.tsx` or the GLB through `file://`. The viewer loads the model over HTTP. Its dependencies and minimum Node version come from `viewer/package.json` and the lockfile. For build validation, run `npm run build` from `viewer/`; `npm run lint` runs the configured source linter.

`package_viewer.py` requires all four fixed-view PNGs and matching image/master receipts in `deliverables/stills/`, plus a matching current master and raw browser GLB receipt. It stages cameras, materials and terrain in `viewer/public/models/`, converts the stills to quality-94 JPEGs, and copies the three attributed references. Missing references are reported as pending and leave the comparison incomplete.

For known opaque surfaces, preparation applies the recorded mean linear material colors before optimization. It retains leaf masks and their textures. The optimizer disables further simplification, palette generation, texture compression and position quantization; a separate Meshopt step preserves decoded FLOAT32 position bytes. `scene/viewer-package.json` binds the optimized GLB to its source, palette and master and records the full-precision check. See [viewer materials](viewer-materials.md) and [browser performance](viewer-performance.md).

The film is copied when available; preview packaging removes a stale staged film when the source film is absent. For final delivery, run `python3 scripts/package_viewer.py --require-film` to reject a missing encoded movie, then inspect every tab in the built viewer. Packaging validates source and still receipts before staging, but a later optimizer failure can leave copied assets; inspect the package receipt and staged files before publishing.

The historical v06 viewer check at `5bf2a27` passed lint, production build, local navigation and a cached `npm ci --offline` installation of 746 packages. That does not promise installation without a populated cache. The current local v07c preview subsequently passed the bounded checks in [browser verification](viewer-verification.md), including saved cameras, walking/reset, stills/comparisons and fullscreen behavior. All 12 final viewer payload files passed packaging, including the complete film. The rebuilt production preview passed its final movie check: six seconds, 1280 × 720, ready state 4, no video error and no browser warning/error entries. The [public walkthrough](https://protolabs-campus-mknutso.mknutso2.chatgpt.site) was deployed on September 8, 2026 with the user's explicit authorization; [the publication record](site-publication.md) identifies the saved version and verification scope. GitHub remains private.
...
The local source remains on `feat/reference-lighting`. [PR #7](https://github.com/mknutso-2/protolabs-campus-reconstruction/pull/7) and [PR #8](https://github.com/mknutso-2/protolabs-campus-reconstruction/pull/8) are merged. [PR #9](https://github.com/mknutso-2/protolabs-campus-reconstruction/pull/9) contains final v07 delivery changes. Preserve the source-tree mappings and visual-check notes when comparing its integrated main tree. The bootstrap identifiers above remain historical records.
...
The clean static source at `e29ad4b9` passed generation, export, geometry and four-image inspection with pinned derived JSON, the bundled font and verified material cache; [its exact record](fresh-checkout-v07c.md) also records source continuity through later changes. The motion sample/endpoints and complete film are accepted with documented limits, current local viewer checks passed, and the final movie loaded correctly in the rebuilt production preview. The actual delivery archive passed extraction and viewer installation/build verification, with all 202 original files unchanged; [delivery verification](delivery-verification.md) records the candidate and final-manifest scope. The [public walkthrough](https://protolabs-campus-mknutso.mknutso2.chatgpt.site) was deployed on September 8, 2026 with the user's explicit authorization; [the publication record](site-publication.md) identifies the saved version and verification scope. GitHub remains private. CPU scene generation and the browser walkthrough are the selected delivery paths. A complete 174 MB raw-research reacquisition/reprocessing and a UE5 application remain optional, separately unverified follow-up work. Preserve the candid fidelity limits after the delivery checks are complete.
