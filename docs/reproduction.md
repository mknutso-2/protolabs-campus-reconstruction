...
The current v07 work is on local branch `feat/reference-lighting`. Connector mirroring through the gentler camera-path commit `69a7594` has been verified by exact source-tree equality; later documentation and delivery changes are tracked separately until synchronized. Preserve source/tree mappings and visual-check notes, and verify the final main tree after merging the staged PRs.
