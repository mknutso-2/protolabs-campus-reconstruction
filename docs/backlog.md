...
- [x] Inspect the final movie in the rebuilt local viewer. The six-second 1280 × 720 movie loaded with ready state 4 and no video or browser-log error; the production build passed. The later public deployment has its own verification record below.
...
- [x] Publish the [public viewer](https://protolabs-campus-mknutso.mknutso2.chatgpt.site) after the user explicitly authorized public access. Sites version 1 deployed successfully on September 8, 2026; see [publication verification](site-publication.md) for the exact public-access checks. The previous owner-only plan was superseded; GitHub remains private.
...
- Final v07 source and delivery integration were merged in [PR #9](https://github.com/mknutso-2/protolabs-campus-reconstruction/pull/9), with exact source-tree comparison to main. Normal Git CLI authentication/history reconciliation is documented; connector mirroring preserves the source contents.
