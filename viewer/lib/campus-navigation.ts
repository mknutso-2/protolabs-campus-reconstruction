...
const footprints: readonly (readonly Point[])[] = [
  [[3.5, 1.1], [61, 1.1], [61, 49.8], [3.5, 49.8]],
  [[-1, 0.7], [3.5, 0.7], [3.5, 8], [-1, 8]],
  [[10, 47], [34, 47], [34, 58], [10, 58]],
  [[55.9, 0], [72.5, 8.5], [76.5, 11.3], [76.5, 24.15], [65.65, 50.03], [56, 47]],
  [[76.66, 24.16], [104.65, 38.86], [92.16, 63.5], [65.65, 50.03]],
  [[71.75, 2], [76.25, 2], [76.25, 7], [71.75, 7]],
];

function insideOrNear(x: number, y: number, polygon: readonly Point[], radius: number) {
  let inside = false;
...
export function canWalkTo(east: number, north: number) {
  return Number.isFinite(east) && Number.isFinite(north)
    && east > -160 && east < 230 && north > -150 && north < 200
