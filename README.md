...
The four **v06** stills were rendered, inspected and preserved with their master, models and snapshot manifest under `deliverables/iterations/v06/`. Their synthetic surface response prompted **v07** work on scanned materials, low-sun lighting, coated glazing, entrance details and generic vehicles. The latest inspected integrated probe was rejected for its sparse distant horizon and oversized lower entrance canopy. A separate fascia fit and three isolated tone variants were inspected; the selected variant uses AgX Medium High Contrast, exposure −0.35 and gamma 1.05, with darker asphalt and greener grass. Updated lettering, a narrow secondary door, fuller tree assets and revised distant context still require the next integrated render. All four production views and motion acceptance remain pending. No v06 motion sequence was rendered; the current-revision sample, complete film, private hosted viewer and final archive remain pending. The latest working branch is `feat/reference-lighting`, currently local until its source is mirrored to GitHub.
...
The fuller three-variant tree library passed isolated geometry, deterministic reduction and GLB checks at 10,000 leaf cards per source tree. Those checks do not establish its appearance in the campus: the next integrated render and distant woodland research remain outstanding. The nearby north-context lidar constraints and open wetland areas must be preserved while refining the farther horizon.
